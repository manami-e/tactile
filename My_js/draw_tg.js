...
**********************/
function draw_tri() {
  //三角形はマウスを押し込んだ後にドラッグし、離すことで描画される
  //sx,syは初めに押し込んだ座標、lx,lyは離した座標となる
  let sx = 0, sy = 0;
  let lx = 0, ly = 0;
  //rectという名の変数が四角形(path)となる
  ...
  draw.off('mousedown').on('mousedown', function (e) {
    if (e.button === 0) {
      ...
      if ($('input[name="draw_path_fillRadio"]:checked').val() === 'custom') rect.fill($('#draw_fill_color').val());
      if ($('input[name="stroke"]:checked').attr('id') === 'radio_dotted_path') {
        tri.attr({ 'stroke-dasharray': PS_WIDTH * $('#dottedLine_line').val() + ' ' + PS_WIDTH * $('#dottedLine_space').val() });
      }
      tri.addClass('connected').addClass('path').back();
      for (let i = 0; i < back_num; i++) tri.forward();
