...
function upload_handle() {
  ...
  $('#table_seiretsu , #table_select_fill').hide(); //レイヤーや塗りつぶし
  ...
  if (draw.select('.edit_select').first() !== undefined) {
    ...
    **********************************************************************/
    ...
    $('#table_seiretsu').show(); //レイヤー設定欄の表示
    ...
    **/
    ...
    let l_resize = SVG.get('l_resize').show(); //ひだり
    ...
    $('#button_right , #button_left , #button_up , #button_down, #button_Xsentor, #button_Ysentor').click(function (e) {
      // 整列テスト
      switch (this.id) {
        case 'button_right': //
          draw.select('.edit_select, .Segments_Group').each(function (i, children) {
            let seiretsu_info = object_seiretsu('seiretsu_right', e, gX, gY, gWidth, gHeight);
            let seiretsu_mat = seiretsu_info.seiretsu_mat;
            dTx = seiretsu_info.dTx;
            dTy = seiretsu_info.dTy;
            update_editgroup(seiretsu_mat, "seiretsu_right");
          });
          break;
        case 'button_left': //
          draw.select('.edit_select, .Segments_Group').each(function (i, children) {
            let seiretsu_info = object_seiretsu('seiretsu_left', e, gX, gY, gWidth, gHeight);
            console.log("done");
          });
          break;
        case 'button_up': //
          draw.select('.edit_select, .Segments_Group').each(function (i, children) {
            let seiretsu_info = object_seiretsu('seiretsu_up', e, gX, gY, gWidth, gHeight);
          });
          break;
        case 'button_down': //
          draw.select('.edit_select, .Segments_Group').each(function (i, children) {
            let seiretsu_info = object_seiretsu('seiretsu_down', e, gX, gY, gWidth, gHeight);
          });
          break;
        case 'button_Xsentor': //
          draw.select('.edit_select, .Segments_Group').each(function (i, children) {
            let seiretsu_info = object_seiretsu('seiretsu_Xsentor', e, gX, gY, gWidth, gHeight);
          });
          break;
        case 'button_Ysentor': //
          draw.select('.edit_select, .Segments_Group').each(function (i, children) {
            let seiretsu_info = object_seiretsu('seiretsu_Ysentor', e, gX, gY, gWidth, gHeight);
          });
          break;
        default:
      }
...
function get_affinmat(type, event, gX, gY, gWidth, gHeight, anchorX, anchorY, dTx, dTy) {
  ...
  // console.log(gX, gY, gWidth, gHeight, anchorX, anchorY, dTx, dTy);
...
//整列
function object_seiretsu(type, event, gX, gY, gWidth, gHeight) {
  draw.select('.edit_select').each(function (i, children) {
    let matrix = this.transform('matrix');
    let trans_matrix = [[matrix.a, matrix.c, matrix.e]
      , [matrix.b, matrix.d, matrix.f]
      , [0, 0, 1]]
    if (this.hasClass('ink') || this.hasClass('braille')) {  //text、image要素の場合
      let px = 0, py = 0;
      px = Number(matrix.e), py = Number(matrix.f); //テキストの座標位置
      let dx = gX - px, dy = gY - py;
      let dy_down = gY + gHeight - py;
      // console.log($('#textbox_resize_braille').val());
      if (type === 'seiretsu_right') this.attr('x', dx); //点字だけ右揃えできない
      else if (type === 'seiretsu_left') this.attr('x', dx);
      else if (type === 'seiretsu_up') { //ptをpxに換算する必要あり
        if (this.hasClass('braille')) this.attr('y', dy + Number($('#textbox_resize_braille').val()) * 1.333);
        else if (this.hasClass('ink')) this.attr('y', dy + Number($('#textbox_resize_ink').val()) * 1.333);
      }
      else if (type === 'seiretsu_down') this.attr('y', dy_down);
      else if (type === 'seiretsu_Xsentor') this.attr('y', dy_down);
      else if (type === 'seiretsu_Ysentor') this.attr('y', dy_down);

    } else if (this.hasClass('circle')) { //円要素の場合
      let cx = Number(this.attr('cx')), cy = Number(this.attr('cy'));
      let cr = Number(this.attr('r'));
      if (type === 'seiretsu_left') {
        let dcx = gX - cx + cr;
        this.attr({
          'cx': cx + dcx,
          'cy': cy,
      ...
      } else if (type === 'seiretsu_right') {
        let dcx = cx - gX - gWidth + cr;
        this.attr({
          'cx': cx - dcx,
          'cy': cy,
          'r': cr
        });
      } else if (type === 'seiretsu_up') {
        let dcy = gY - cy + cr;
        this.attr({
          'cx': cx,
          'cy': cy + dcy,
      ...
      } else if (type === 'seiretsu_down') {
        let dcy = cy - gY - gHeight + cr;
        this.attr({
          'cx': cx,
          'cy': cy - dcy,
          'r': cr
        });
      } else if (type === 'seiretsu_Xsentor') {
        let dcx = (gX + gX + gWidth) / 2 - cx;
        this.attr({
          'cx': cx + dcx,
          'cy': cy,
          'r': cr
        });
      } else if (type === 'seiretsu_Ysentor') {
        let dcy = (gY + gY + gHeight) / 2 - cy;
        this.attr({
          'cx': cx,
          'cy': cy + dcy,
          'r': cr
      ...
      let dpoint = this.clear().array().settle(); //pathのdpoint配列を取得
      if (type === 'seiretsu_right') {
        let d = "";
        let max = dpoint[0][1];
        for (let j = 0; j < dpoint.length; j++) {
          if (dpoint[j][0] !== "Z") {
            if (max < Number(dpoint[j][1])) {
              max = dpoint[j][1];
            }
          }
        }
        let dx = gX + gWidth - max;
        for (let j = 0; j < dpoint.length; j++) {
          if (dpoint[j][0] !== "Z") {  //属性がZ以外の場合 j0はMかLかZ
            let transx = dpoint[j][1] + dx;
            d += dpoint[j][0] + " " + transx + " " + dpoint[j][2]; //新しい座標として格納
          } else {
            d += dpoint[j][0];
          }
        }
        this.attr({ 'd': d });
      } else if (type === 'seiretsu_left') {
        let d = "";
        let min = dpoint[0][1];
        for (let j = 0; j < dpoint.length; j++) {
          if (dpoint[j][0] !== "Z") {
            if (min > Number(dpoint[j][1])) {
              min = dpoint[j][1];
            }
          }
        }
        ...
        for (let j = 0; j < dpoint.length; j++) {
          if (dpoint[j][0] !== "Z") {  //属性がZ以外の場合 j0はMかLかZ
            let transy = dpoint[j][2] + dy;
            d += dpoint[j][0] + " " + dpoint[j][1] + " " + transy; //新しい座標として格納
          } else {
            d += dpoint[j][0];
          }
        }
        ...
        for (let j = 0; j < dpoint.length; j++) {
          if (dpoint[j][0] !== "Z") {
            if (max < Number(dpoint[j][2])) {
              max = dpoint[j][2];
            }
          }
        }
        let dy = (gY + gHeight) - max;
        for (let j = 0; j < dpoint.length; j++) {
          if (dpoint[j][0] !== "Z") {  //属性がZ以外の場合 j0はMかLかZ
            let transy = dpoint[j][2] + dy;
            d += dpoint[j][0] + " " + dpoint[j][1] + " " + transy; //新しい座標として格納
        ...
        for (let j = 0; j < dpoint.length; j++) {
          if (dpoint[j][0] !== "Z") {
            if (max < Number(dpoint[j][1])) {
              max = dpoint[j][1];
            } else if (min > Number(dpoint[j][1])) {
              min = dpoint[j][1];
            }
          }
        }
        let dx = ((gX + gX + gWidth) / 2) - ((max + min) / 2);
        for (let j = 0; j < dpoint.length; j++) {
          if (dpoint[j][0] !== "Z") {  //属性がZ以外の場合 j0はMかLかZ
            let transx = dpoint[j][1] + dx;
            d += dpoint[j][0] + " " + transx + " " + dpoint[j][2]; //新しい座標として格納
        ...
        for (let j = 0; j < dpoint.length; j++) {
          if (dpoint[j][0] !== "Z") {
            if (max < Number(dpoint[j][2])) {
              max = dpoint[j][2];
            } else if (min > Number(dpoint[j][2])) {
              min = dpoint[j][2];
            }
          }
        }
        let dy = ((gY + gY + gHeight) / 2) - ((max + min) / 2);
        for (let j = 0; j < dpoint.length; j++) {
          if (dpoint[j][0] !== "Z") {  //属性がZ以外の場合 j0はMかLかZ
            let transy = dpoint[j][2] + dy;
            d += dpoint[j][0] + " " + dpoint[j][1] + " " + transy; //新しい座標として格納
